...
- broadcast: 发送全服公告
- read_url: 获取指定URL的网页文本内容（如Spark性能报告页面 spark.lucko.me/xxx）。大小限制100KB，超时15秒
...
- 注意：profiler 是异步操作。使用 --timeout 时，URL 不会立即出现在命令输出中，需在超时后用 read_log 搜索 "spark.lucko.me" 获取报告链接
- 拿到 spark.lucko.me 的报告链接后，用 read_url 工具读取页面内容，分析其中的 TPS、CPU、内存等性能数据，然后给出诊断建议
...
**重要**：工具执行完成后，你必须基于工具返回的结果（tool_result）主动总结发现、给出分析，不要等用户追问。如果 read_url 或其他工具返回了数据，立即解读并报告关键信息。"""
...
TOOLS = [
    ...
    {
        "name": "read_url",
        "description": "获取指定URL的网页文本内容。用于读取Spark性能报告页面（spark.lucko.me）、在线文档等网页内容。返回纯文本格式，大小限制100KB。",
        "input_schema": {
            "type": "object",
            "properties": {
                "url": {"type": "string", "description": "要读取的完整URL，如 https://spark.lucko.me/xxxx"}
            },
            "required": ["url"]
        }
    }
...
def _content_blocks_to_dicts(content_blocks) -> list[dict]:
    """将 Anthropic SDK 的 ContentBlock 对象列表转为可 JSON 序列化的 dict 列表"""
    result = []
...
class AIAgent:
    ...
    async def process_message(self, user_message: str, server_id: str, current_status: dict = None, query_only: bool = False, model_tier: Optional[str] = None, admin_id: str = "admin", system_prompt: str = None) -> Dict[str, Any]:
        ...
        assistant_msg = {"role": "assistant", "content": _content_blocks_to_dicts(response.content)}
    ...
    async def continue_after_tools(
        ...
        try:
            ...
            logger.error(f"工具后续 LLM 调用失败(无可用供应商): {e}")
            return {"text": f"⚠️ AI 分析服务暂不可用：{e}", "degraded": False}
        except Exception as e:
            logger.error(f"工具后续 LLM 调用失败: {e}", exc_info=True)
            return {"text": f"⚠️ AI 分析请求失败：{e}", "degraded": False}

        content_dicts = _content_blocks_to_dicts(response.content)
        self.conversation_history[hkey].append({"role": "assistant", "content": content_dicts})
        self._trim_history(hkey)
    ...
    def clear_conversation(self, admin_id: str, server_id: str):
        """清除指定用户+服务器的对话历史"""
        hkey = (admin_id, server_id)
        self.conversation_history.pop(hkey, None)
        self._access_order.pop(hkey, None)
        logger.info(f"[{server_id}] 对话历史已清除 (user={admin_id})")
