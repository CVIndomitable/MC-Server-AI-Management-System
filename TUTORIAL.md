...
选择服务器后进入聊天界面，这是你与 AI 管理员交互的主要方式。你可以用自然语言描述你想做的事情：
...
| Standard | 含"为什么""怎么办"等关键词，或长消息 | "怎么回事"、批量操作 |
...
记忆会自动注入 AI 的 System Prompt，使 AI 能够记住跨会话的上下文。可通过 API 管理记忆条目。
...
**服务器角色**：
- `owner` — 主管理员（首个绑定者），可审批申请、解绑用户、重命名服务器
...
3. 确认 `ANTHROPIC_BASE_URL` 指向正确的 API 网关
...
4. 打开浏览器 → 登录 → 选择/绑定服务器 → 开始用自然语言管理服务器
```
